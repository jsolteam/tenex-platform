...
func main() {
	...
	if err := c.Start(ctx); err != nil {
		_, _ = os.Stderr.WriteString("scheduler: container start failed: " + err.Error() + "\n")
		os.Exit(1)
	}

	facade.L().Info("scheduler: started")

	<-ctx.Done()

	facade.L().Info("scheduler: shutting down")
	...
	if err := c.Stop(shutCtx); err != nil {
		facade.L().Error("scheduler: shutdown error", zap.Error(err))
		os.Exit(1)
	}

	facade.L().Info("scheduler: shutdown complete")
}
