...
func (r *Repository) GetByID(ctx context.Context, id int64) (*watcher.Watcher, error) {
	...
	start := time.Now()
	ctx, span := r.tracer.Start(ctx, "watcherrepo."+method)
	defer span.End()
	...
	if err != nil {
		appErr := apperrors.DB("watcherrepo."+method, err)
		repolog.Err(ctx, r.log, span, r.met, repoName, method, "failed to get watcher", appErr, zap.Int64("id", id))
		return nil, appErr
	}
	return w, nil
}
...
func (r *Repository) GetByPair(ctx context.Context, userID, watcherUserID int64) (*watcher.Watcher, error) {
	...
	if err != nil {
		appErr := apperrors.DB("watcherrepo."+method, err)
		repolog.Err(ctx, r.log, span, r.met, repoName, method, "failed to get watcher by pair", appErr, zap.Int64("user_id", userID))
		return nil, appErr
	}
	return w, nil
}
...
func (r *Repository) ListByUser(ctx context.Context, userID int64) ([]watcher.Watcher, error) {
	...
	if err != nil {
		appErr := apperrors.DB("watcherrepo."+method, err)
		repolog.Err(ctx, r.log, span, r.met, repoName, method, "failed to list watchers", appErr, zap.Int64("user_id", userID))
		return nil, appErr
	}
	return result, nil
}
...
func (r *Repository) ListByWatcher(ctx context.Context, watcherUserID int64) ([]watcher.Watcher, error) {
	...
	if err != nil {
		appErr := apperrors.DB("watcherrepo."+method, err)
		repolog.Err(ctx, r.log, span, r.met, repoName, method, "failed to list watched users", appErr, zap.Int64("watcher_user_id", watcherUserID))
		return nil, appErr
	}
	return result, nil
}
...
func (r *Repository) Create(ctx context.Context, w *watcher.Watcher) error {
	...
	if err != nil {
		if isUniqueViolation(err) {
			appErr := apperrors.Validation("watcherrepo."+method, watcher.ErrAlreadyExists)
			repolog.Err(ctx, r.log, span, r.met, repoName, method, "watcher pair already exists", appErr,
				zap.Int64("user_id", w.UserID), zap.Int64("watcher_user_id", w.WatcherUserID))
			return appErr
		}
		appErr := apperrors.DB("watcherrepo."+method, err)
		repolog.Err(ctx, r.log, span, r.met, repoName, method, "failed to create watcher", appErr, zap.Int64("user_id", w.UserID))
		return appErr
	}
	return nil
}
...
func (r *Repository) Delete(ctx context.Context, id int64) error {
	...
	if err != nil {
		appErr := apperrors.DB("watcherrepo."+method, err)
		repolog.Err(ctx, r.log, span, r.met, repoName, method, "failed to delete watcher", appErr, zap.Int64("id", id))
		return appErr
	}
...
func (r *Repository) GetNotificationByID(ctx context.Context, id int64) (*watcher.WatcherNotification, error) {
	const method = "GetNotificationByID"
	start := time.Now()
	ctx, span := r.tracer.Start(ctx, "watcherrepo."+method)
	defer span.End()
	defer func() { r.met.RecordDuration(ctx, repoName, method, time.Since(start).Seconds()) }()
	...
	if err != nil {
		appErr := apperrors.DB("watcherrepo."+method, err)
		repolog.Err(ctx, r.log, span, r.met, repoName, method, "failed to get watcher notification", appErr, zap.Int64("id", id))
		return nil, appErr
	}
	return n, nil
}
...
func (r *Repository) ListPendingNotifications(ctx context.Context, limit int) ([]watcher.WatcherNotification, error) {
	const method = "ListPendingNotifications"
	start := time.Now()
	ctx, span := r.tracer.Start(ctx, "watcherrepo."+method)
	defer span.End()
	defer func() { r.met.RecordDuration(ctx, repoName, method, time.Since(start).Seconds()) }()
	...
	if err != nil {
		appErr := apperrors.DB("watcherrepo."+method, err)
		repolog.Err(ctx, r.log, span, r.met, repoName, method, "failed to list pending notifications", appErr)
		return nil, appErr
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err != nil {
			appErr := apperrors.DB("watcherrepo."+method+".scan", err)
			repolog.Err(ctx, r.log, span, r.met, repoName, method, "failed to scan notification", appErr)
			return nil, appErr
		}
		result = append(result, *n)
	}
	if err = rows.Err(); err != nil {
		appErr := apperrors.DB("watcherrepo."+method+".rows", err)
		repolog.Err(ctx, r.log, span, r.met, repoName, method, "notifications rows error", appErr)
		return nil, appErr
	}
	return result, nil
}
...
func (r *Repository) CreateNotification(ctx context.Context, n *watcher.WatcherNotification) error {
	const method = "CreateNotification"
	start := time.Now()
	ctx, span := r.tracer.Start(ctx, "watcherrepo."+method)
	defer span.End()
	defer func() { r.met.RecordDuration(ctx, repoName, method, time.Since(start).Seconds()) }()
	...
	if err != nil {
		appErr := apperrors.DB("watcherrepo."+method, err)
		repolog.Err(ctx, r.log, span, r.met, repoName, method, "failed to create watcher notification", appErr,
...
func (r *Repository) MarkNotificationSent(ctx context.Context, id int64, sentAt time.Time) error {
	const method = "MarkNotificationSent"
	start := time.Now()
	ctx, span := r.tracer.Start(ctx, "watcherrepo."+method)
	defer span.End()
	defer func() { r.met.RecordDuration(ctx, repoName, method, time.Since(start).Seconds()) }()
	...
	if err != nil {
		appErr := apperrors.DB("watcherrepo."+method, err)
		repolog.Err(ctx, r.log, span, r.met, repoName, method, "failed to mark notification sent", appErr, zap.Int64("id", id))
		return appErr
	}
	if n, _ := res.RowsAffected(); n == 0 {
